...
double YanusLagDeviation(double H) {
    ...
    //СКО ГАЛ
    double YanusSKO = Czv * (1/(3.5 * (H/Czv * cos(psiIzl)))) / (2 * f * sin(psiIzl));
    
    //белый шум с заданнымСКО
    YanusSKO = noise(YanusSKO);
...
double redNoise(double SKO)
{
    ...
    red = (b0 + b1 + b2 + b3 + b4 + b5 + b6 + (2 * ((rand()/(static_cast<double>(RAND_MAX))) - 0.5))*0.5362) * SKO;
...
double noise (double K) {
    return K - rand()/(double(RAND_MAX)) * 2 * K;
}
